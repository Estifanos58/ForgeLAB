...
6. **Docker Compose Rewrite Networking Mismatch:** In `frontend/next.config.js`, API rewrites target `http://localhost:8080/api/:path*`. When running inside Docker Compose containers, `localhost:8080` resolves to the frontend container's loopback interface rather than the `backend` service (`http://backend:8080`). As documented in [docs/14-known-limitations.md](14-known-limitations.md), this networking configuration requires reconciliation before the containerized stack can function as an end-to-end development environment.
